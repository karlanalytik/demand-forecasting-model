...
Script to make inference using the generated model.
"""
...
def load_inference_data(input_path: str) -> pd.DataFrame:
    """
    Load inference data from several of CSV files and integrate a unique 
    dataframe.
    """
    catalog_path = PROJECT_ROOT / RAW_DATA_PATH
    old_data_path = PROJECT_ROOT / PREP_DATA_PATH

    sales_pred = pd.read_csv(input_path)
    sales = pd.read_csv(os.path.join(old_data_path, 'sales_prep.csv'))
    items = pd.read_csv(os.path.join(catalog_path, 'items.csv'))
    cats  = pd.read_csv(os.path.join(catalog_path, 'item_categories.csv'))
    ...
    cols = [col for col in sales.columns if col not in ['date_block_num', 
                                                        'item_category_id', 
                                                        'item_cnt_month']]
    sales = sales[sales['date_block_num'] == max(sales['date_block_num'])][cols]
    #Aquí definir month

    df = pd.merge(df, sales, how = 'left', on = ['shop_id', 'item_id'])
